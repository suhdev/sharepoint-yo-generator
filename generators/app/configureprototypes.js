...
function readPrototypesRecursively(prototypeTemplatesDir,prototypes){
    if (fs.existsSync(prototypeTemplatesDir)) {
        console.log('Prototypes folder found');
        var files = fs.readdirSync(prototypeTemplatesDir);
        console.log(`Prototypes files found: ${files.length}`);
        var directories = []; 
        ...
        .map((file) => {
            ...
            if (fs.existsSync(path.resolve(prototypeTemplatesDir, path.basename(file,path.extname(file)) + '.json'))) {
                try {
                    p = Object.assign({}, p, require(path.resolve(prototypeTemplatesDir, path.basename(file,path.extname(file)) + '.json')));
                    p.definitionFullPath = path.resolve(prototypeTemplatesDir, path.basename(file,path.extname(file)) + '.json'); 
                } catch (err) {
                    console.log(`An error has occured while getting prototype definition file: ${err.message}`);
                }
            }
...
module.exports = function configurePrototypes(generator,config){
    ...
    readPrototypesRecursively(
        path.resolve(generator.destinationPath(config.prototypeTemplatesDir)),prototypes); 
    const prompts = [
        {
            type:'list',
            name:'action',
            ...
            name:'protoName', 
            message: (answers)=>{
                return answers.action === 'remove prototype'?'Which prototype do you want to delete?':'Which prototype do you want to edit?'
            },
            when(answers){
                return answers.action === 'remove prototype' ||
                    answers.action === 'edit prototype';
            },
            choices:()=>{
            ...
            default:(answers)=>{
                return path.basename(answers.protoName) === path.dirname(answers.protoName).split(path.sep).pop();
            }
        },
        {
            type: 'confirm',
            ...
            when(answers) {
                ...
                const srcPath = path.resolve(config.srcDir, './prototypes', path.basename(answers.protoName, '.ts'));
                let srcFile = fs.existsSync(srcPath)?srcPath:fs.existsSync(srcPath+'x')?srcPath+'x':false;  
        ...
        .then((answers)=>{
            ...
            if (action === 'create prototype'){
                ...
                try {
                    ...
                    console.log(`An error has occured while deleting prototype ${p.name}: ${err.message}`)
                }
            }
        })
        .then(()=>{
